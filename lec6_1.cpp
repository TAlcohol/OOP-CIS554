...
// all the STL containers, are support call by value/reference,
// but array only supports call by reference
...
ostream& operator<<(ostream& str, const vector<ThreeD>& V) {
    str << "[";
    for (ThreeD i: V) { str << i << ", "; }
    // for (auto i: V) { str << i << ", "; } // use auto is ok, because we
    // have already defined V
    str << "]";
    return str;
}
...
int main() {
	...
	cout << endl;
    cout << M1[8] << endl; // will print Sanjay
    ...
    // base on key, delete something in a map
    // only map allow key based erase
    // list and vector iterate over address
    // list and vector only support you put in iterator
    // map supports iterate and elememt
    // like M2.erase(it5) or M2.erase(t1)
    // program won't crash if t1 don't exist
    M2.erase(t1);

    // after erase, what are the pointer/iterator point to?
    // point to the one follow it

    // stack assoicated with scope, when scope ends, it will be removed
    // global variables are in heap. like "new xxxxx"
    ...
    // int AA[w1];
    // this will crash
    // because the size of array always has to be constant
    // you can't do it in regular array
    int *p10 = new int[w1];
    // but you can do it in dynamic array
	// the size of a dynamically created array can be specified by a varible
    // dynamic allocation won't allocate during compile process
    // but for regular array, it should know the size of array
...
// nowadays, don't use NULL, use nullptr
// Because except meaning point to nullptr, NULL also carry the meaning of 0
// if you cout << NULL, it will gives you 0